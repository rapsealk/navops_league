...
writer = SummaryWriter(f'runs/{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}-pong')
...
for it in range(100000):
    ...
    writer.add_scalar('reward', np.sum(discounted_rewards), it)
    writer.add_scalar('loss', np.mean(losses), it)
