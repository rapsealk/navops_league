...
c_sampling_ratio = 1.0   # For truncating importance sampling ratio
...
def train(model, optimizer, memory, on_policy=False):
    ...
    rho_bar = rho_a.clamp(max=c_sampling_ratio)
    correction_coeff = (1-c_sampling_ratio/rho).clamp(min=0)

    q_retrace = v[-1] * done_mask[-1]
    q_retrace_lst = []
    for i in reversed(range(len(r))):
        ...
        if is_first[i] and i != 0:
            q_retrace = v[i-1] * done_mask[i-1]     # When a new sequence begins, q_retrace is initialized

    q_retrace_lst.reverse()
    q_retrace = torch.tensor(q_retrace_lst, dtype=torch.float).unsqueeze(1)
...
def main():
    ...
    for n_epi in range(10000):
        ...
        if n_epi % print_interval == 0 and n_epi != 0:
            print(f'# of episode :{n_epi}, avg score : {(score/print_interval):.1f}, buffer size : {memory.size()}')
            score = 0.0
